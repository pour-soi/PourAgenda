...
import { disableBrowserPush } from "@/lib/push-subscription";
...
export function SettingsManager({
  ...
  async function disablePersonalAppointmentPush() {
    setPending(true); setMessage("");
    try {
      const registration = await navigator.serviceWorker.ready;
      await disableBrowserPush(registration, async (endpoint) => {
        const query = supabase.from("push_subscriptions");
        const result = endpoint
          ? await query.delete().eq("user_id", userId).eq("endpoint", endpoint)
          : await query.update({ disabled_at: new Date().toISOString() }).eq("user_id", userId).is("disabled_at", null);
        if (result.error) throw result.error;
      });
      setPushState("default");
      setMessage("Personal appointment notifications disabled.");
    } catch {
      setMessage("Personal appointment notifications could not be disabled.");
    } finally {
      setPending(false);
    }
  }
      ...
      <form onSubmit={saveSettings} className="mt-8 space-y-5 rounded-[var(--radius)] border border-border bg-surface p-5 sm:p-6">
        ...
        <fieldset className="rounded-lg border border-border p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {pushState === "enabled"
              ? <button type="button" disabled={pending} onClick={() => void disablePersonalAppointmentPush()} className="rounded-lg border border-border px-3">Disable notifications</button>
              : <button type="button" disabled={pending || pushState === "blocked" || pushState === "unsupported"}
                onClick={() => void enablePersonalAppointmentPush()} className="rounded-lg border border-border px-3">Enable notifications</button>}
          </div>
        </fieldset>
