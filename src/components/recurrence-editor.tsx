...
export function RecurrenceEditor({ appointment, exceptions, frequency, interval, until, timezone, persisted,
  ...
  onSkip: (item: RecurrencePreviewItem) => void; onRestore: (item: RecurrencePreviewItem) => void;
  onEdit: (item: RecurrencePreviewItem) => void; onMove: (item: RecurrencePreviewItem) => void;
}) {
  ...
  const act = (item: RecurrencePreviewItem, action: (item: RecurrencePreviewItem) => void) => {
    setMenu(null); action(item);
  };
  ...
  return <fieldset className="sm:col-span-2 rounded-xl bg-background/70 p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      {frequency && <label className="text-sm font-medium">Ends<select aria-label="Repeat ending" value={until ? "date" : "never"} onChange={(event) => onUntil(event.target.value === "date" ? appointment.intended_local_start?.slice(0,10) ?? appointment.starts_at.slice(0,10) : "")} className="mt-1 w-full rounded-lg border border-border bg-surface px-3"><option value="never">Never</option><option value="date">On a date</option></select></label>}
      {frequency && until && <div><span className="text-sm font-medium">End Date</span><EnglishDateTimePicker ariaLabel="Repeat end date" value={until} dateOnly min={appointment.intended_local_start ?? appointment.starts_at} onChange={(value) => onUntil(value.slice(0,10))}/></div>}
    </div>
    {frequency && <div className="mt-4 border-t border-border/70 pt-4" aria-live="polite"><h3 className="text-sm font-semibold">Summary</h3><p className="mt-1 font-medium">{recurrenceSummary(appointment).replace(/^Repeats /, "").replace(/ until .*| and never ends\.$/, "")}</p><p className="text-sm text-muted">{until ? `Until ${formatDate(`${until}T12:00:00Z`, "UTC")}` : "No end date"}</p>
      <h3 className="mt-4 text-sm font-semibold">Upcoming</h3><div className="mt-1 divide-y divide-border/70">{preview.map((item) => {
        const id=item.occurrence.occurrence_id; const label=formatDate(item.occurrence.starts_at,item.occurrence.all_day?"UTC":timezone);
        return <div key={id} className="relative flex min-h-12 items-center justify-between gap-3 py-2"><div><span className={item.state === "skipped" ? "text-muted line-through" : "font-medium"}>{item.state === "moved" ? `${formatDate(item.originalStartsAt, timezone)} → ${label}` : label}</span>{item.state !== "normal" && <span className="ml-2 text-xs font-semibold uppercase tracking-wide text-muted" role="status">{item.state}</span>}</div>{persisted && <button ref={(node)=>{if(node)triggers.current.set(id,node);}} type="button" aria-label={`More actions for ${label}`} aria-haspopup="menu" aria-expanded={menu===id} onClick={()=>setMenu(menu===id?null:id)} className="grid size-11 place-items-center rounded-full hover:bg-surface focus-visible:outline-2 focus-visible:outline-primary"><MoreHorizontal aria-hidden="true"/></button>}{menu===id && <div ref={menuRef} role="menu" onKeyDown={navigateMenu} className="absolute right-0 top-11 z-20 min-w-56 rounded-lg border border-border bg-surface p-1 shadow-xl">{item.state === "skipped" ? <button role="menuitem" type="button" onClick={()=>act(item,onRestore)} className="min-h-11 w-full rounded-md px-3 text-left hover:bg-background">Restore occurrence</button> : <><button role="menuitem" type="button" onClick={()=>act(item,onSkip)} className="min-h-11 w-full rounded-md px-3 text-left hover:bg-background">Skip this occurrence</button><button role="menuitem" type="button" onClick={()=>act(item,onEdit)} className="min-h-11 w-full rounded-md px-3 text-left hover:bg-background">Edit only this occurrence</button><button role="menuitem" type="button" onClick={()=>act(item,onMove)} className="min-h-11 w-full rounded-md px-3 text-left hover:bg-background">Move this occurrence</button></>}</div>}</div>})}</div>{!persisted && <p className="mt-2 text-xs text-muted">Save the series before managing individual occurrences.</p>}</div>}
  </fieldset>;
}
